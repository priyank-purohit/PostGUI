...
exports.getQBFilters = function(dbIndex, table, columns) {
	...
	for (let i = 0; i < columns.length; i++) {
		plain_strings_query_builder.push({ id: columns[i], label: this.getColumnConfig(dbIndex, table, columns[i], "rename"), type: this.getColumnConfig(dbIndex, table, columns[i], "type"), input: this.getColumnConfig(dbIndex, table, columns[i], "input"), values: this.getColumnConfig(dbIndex, table, columns[i], "values"), operators: this.getColumnConfig(dbIndex, table, columns[i], "operators") ? this.getColumnConfig(dbIndex, table, columns[i], "operators") : allSupportedQBFilters });
	}
